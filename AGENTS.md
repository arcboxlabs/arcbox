...
¹ Host cost is the high-water mark of guest-*touched* pages, not the
  ...
  host `phys_footprint` (2026-08-01). On VZ that mark is a one-way
  ratchet — guest RAM lives in Apple's XPC process, out of the daemon's
  reach — so the guest freeing 3GB leaves the host paying for it forever.
  On HV the guest's free page reporting drains it back (measured
  2026-09-25 on a 16 GB VM: 1.9 GB → 5.0 GB while the guest held 3 GiB
  of tmpfs → 1.95 GB ten seconds after it freed it). The mechanism, and
  the `madvise` dead ends, are in `virt/arcbox-vmm/AGENTS.md`
...
- When project conventions or processes change, this file (`AGENTS.md`) must be updated promptly. All changes to this file require human approval. It is the only root instruction file — there is no `CLAUDE.md`; Claude Code reads `AGENTS.md` directly and expands the `@` imports above.
...
- Changes land on `master` directly. Work on `master` or on a local `type/short-description` branch (e.g. `feat/virtio-console`, `fix/dhcp-lease-expiry`) that you fast-forward onto `master` when done, then push. A pull request is opt-in, for work that wants a review. The master ruleset still blocks force-pushes and branch deletion; the repository admin role bypasses its pull-request requirement, a contributor without that role goes through a PR.
- CI runs *after* the push, so run its gates before every push to `master`, cheapest first: `cargo fmt --check`, `cargo clippy --workspace --exclude arcbox-agent -- -D warnings`, `cargo xtask check-layers`, `cargo test --workspace --exclude arcbox-agent`, and `xcrun swift-format lint --strict --parallel --recursive virt/arcbox-vz/shim` when the VZ shim changed. The guest agent is outside every gate — lint it for musl by hand (`guest/AGENTS.md`). A change to the guest agent, the VMM, or the daemon's boot path also gets a daemon-level e2e run on the backend it touches (`app/AGENTS.md` "Validation ladder").
...
- Use `cargo add` / `cargo remove` for dependency changes, not manual Cargo.toml edits. Check the lockfile diff afterwards: `cargo add` has bumped unrelated crates and `cargo remove` has dropped an unrelated workspace dependency — `git checkout Cargo.toml Cargo.lock` and `cargo check` re-adds only the intended edge.
- When a PR is used: the ruleset requires every review thread to be **resolved** (0 approvals required; squash or rebase only — merge commits are disabled). Address or answer each bot/reviewer thread, resolve it (GraphQL `resolveReviewThread` if working from the CLI), then merge normally — do not reach for `--admin`. For stacked PRs, merge the base PR first — and expect GitHub to close the stacked PR when the base branch is deleted; recovery is: re-push the old base ref, reopen the PR, retarget it to master, delete the ref again, then close/reopen once more to trigger CI (pushes made while a PR is closed fire no events).
