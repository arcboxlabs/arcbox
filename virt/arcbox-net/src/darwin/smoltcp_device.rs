...
impl SmoltcpDevice {
    ...
    /// Provides mutable access to the rx_queue for filtering tunnel returns.
    ///
    /// Used by the datapath to remove tunnel-return packets before smoltcp
    /// processes them (e.g. TCP SYN-ACK replies to tunneled connections).
    pub fn rx_queue_mut(&mut self) -> &mut VecDeque<Vec<u8>> {
        &mut self.rx_queue
    }
