...
//! `arcbox` is available from any terminal session.
...
/// Status report for `arcbox setup status --format json`.
#[derive(Serialize)]
struct StatusOutput {
    installed: bool,
...
async fn status(format: OutputFormat) -> Result<()> {
    ...
    let zsh_comp = comp.join("zsh/_arcbox");
    ...
    match format {
        ...
        OutputFormat::Table | OutputFormat::Quiet => {
            ...
            if all_ok {
                ...
                println!("Status: not installed (run `arcbox setup install`)");
            }
        }
    }
...
fn print_completions(shell: ShellKind) {
    ...
    clap_complete::generate(clap_shell, &mut cmd, "arcbox", &mut std::io::stdout());
}

fn generate_all_completions(comp_dir: &Path) -> Result<()> {
    let shells = [
        (clap_complete::Shell::Zsh, comp_dir.join("zsh/_arcbox")),
        (clap_complete::Shell::Bash, comp_dir.join("bash/arcbox")),
        (
            clap_complete::Shell::Fish,
            comp_dir.join("fish/arcbox.fish"),
        ),
    ];
    ...
    for (shell, path) in &shells {
        let mut cmd = Cli::command();
        let mut buf = Vec::new();
        clap_complete::generate(*shell, &mut cmd, "arcbox", &mut buf);
...
# This file is auto-generated by `arcbox setup install`.
...
# This file is auto-generated by `arcbox setup install`.
export PATH="${HOME}/.arcbox/bin:${PATH}"
if [ -f "${HOME}/.arcbox/completions/bash/arcbox" ]; then
    source "${HOME}/.arcbox/completions/bash/arcbox"
fi
"#;
...
mod tests {
    ...
    fn completions_generate_without_panic() {
        ...
        assert!(comp_dir.join("zsh/_arcbox").exists());
        assert!(comp_dir.join("bash/arcbox").exists());
        assert!(comp_dir.join("fish/arcbox.fish").exists());
    }
}
